...
const FakeTab = ({
  ...
  if (groupName) {
    return (
      <TabWrapper>
        <ColorSquare value={groupColor}>{groupName}</ColorSquare>
        <FakeTabWrapper>
          <img src={icon} />
          <div>{name}</div>
        </FakeTabWrapper>
...
const ruleToText = (rule) =>
  `${rule.name}, ${rule.pattern}${rule.color ? ', ' + rule.color : ''}`;

const initialTabs = [
  {
    name: 'Inbox - johndoe@gmail.com',
    icon: GmailIconUrl,
    groupName: 'email',
    groupColor: COLORS.blue,
  },
  {
    name: 'acid tabs extension',
    icon: GoogleIconUrl,
    groupName: null,
    groupColor: null,
  },
...
const TabDemo = (props) => {
  ...
  return (
    <FillColumn>
      <ExampleWrapper>
        <Prompt>
          TabSense lets you automatically{' '}
          <span style={{ color: COLORS.blue, textDecoration: 'underline' }}>
            group
          </span>{' '}
          your tabs with url patterns
        </Prompt>
        <FakeTabsRow>
          {tabs.map((t) => (
            <FakeTab
        ...
        <DisabledRow alignItems="flex-end">
          ...
          <TextField fullWidth disabled label="Name" value="email" />
          <TextField
            fullWidth
            disabled
            label="URL Pattern"
          ...
          <Select
            value={COLORS.blue}
            disabled
            renderValue={(val) => <ColorCircle value={val} displayMode />}
          ></Select>
        ...
        <Prompt style={{ marginTop: '4rem' }}>
          <Button
            color="primary"
            variant="contained"
            style={{ backgroundColor: COLORS.blue, color: DARK_BLUE }}
